...
def merge_image():
    ...
    widths, heights = zip(*(x.size for x in images)) # [(10, 10), (20, 20), (30, 30)] zip으로 각 튜플내 같은 순서 원소를 가져와서 그룹 튜플로 만든다
    ...
    for idx, img in enumerate(images):
        ...
        y_offset += img.size[1] # height 값 만큼 y 위치에 더해줌

        progress = (idx + 1) / len(images) * 100 # 실제 percent를 계산
        p_var.set(progress)
        progress_bar.update()

    dest_path = os.path.join(txt_dest_path.get(), 'new_photo.jpg') # 저장할 경로
