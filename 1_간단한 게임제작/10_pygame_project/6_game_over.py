...
## Pang game(게임 종료: 모든 공 없애면(성공), 캐릭터가 공에 닿으면(실패), 시간 제한 99초 초과(실패))
################################################ (반드시 초기 설정)
pygame.init()
...
while running:
    ...
    # 무기 위치 조정(중요한 개념, 반복 수정으로 값 변동 시키기)
    # 100, 200 -> 180 ,160, 140, ...
    weapons = [ [w[0], w[1] - weapon_speed] for w in weapons] # weapons에 있는 위치 리스트를 w로 하고, weapons에는 w에 weapon_speed를 각각 반복해 뺀 후 집어 넣는다, 무기 위치를 위로
